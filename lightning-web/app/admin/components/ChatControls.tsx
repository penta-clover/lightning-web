...
export function ChatControls({ onSendMessage, getRecommendations, className }: ChatControlsProps) {
  const [shortcuts, setShortcuts] = useState(["대 상 혁", "Siuuuuuuuuuuuu", "와", "화이팅🔥🔥🔥"])
  ...
  useEffect(() => {
    loadDummies((dummies) => { setDummyUsers(dummies) });
  }, []);
  ...
  const sendMessage = (message: string) => {
    ...
    const selectedDummy = dummyUsers[Math.floor(Math.random() * dummyUsers.length)];
      ...
      <CardContent className="space-y-4 px-4 py-2">
        ...
        <div className="space-y-2">
          ...
          <div className="flex space-x-2 items-center w-full">
            <Button disabled={isGenerating} onClick={loadRecommendations} variant="outline" className={clsx("whitespace-nowrap w-full transition-all", {'shadow-lg':isGenerating})}>
            ...
            recommendations.map((recommendation, index) => (
              <div key={index} className="flex space-x-2">
                <AutoResizeTextarea
                  ...
                  onChange={(e) => setRecommendations([...recommendations.slice(0, index), e.target.value, ...recommendations.slice(index + 1)])}
                  className="flex-grow break-all h-8 resize-none"
                />
                <Button onClick={() => { sendMessage(recommendation) }} className="whitespace-nowrap">
                  전송
                </Button>
              </div>
            ))
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-0">
            {shortcuts.map((shortcut, index) => (
              <div key={index} className="flex items-center justify-between shadow-sm bg-secondary rounded border-black">
                <Button
                  className="shadow-sm"
                  size="sm"
                  onClick={() => {
                ...
                <div className="flex space-x-2 items-center">
                  <Button variant="ghost" size="icon" onClick={() => removeShortcut(shortcut)}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              </div>
...
function Inputter(props: { placeholder: string, defaultValue?: string, onSend: (input: string) => void }) {
  ...
  return (<div className="flex space-x-2">
    <Input
      ...
      className="flex-grow"
    />
    <Button onClick={() => { props.onSend(inputMessage); setInputMessage("") }} className="whitespace-nowrap">
      전송
    </Button>
  </div>);
}
